...
export async function syncTripsDown(userId) {
  ...
  try {
    ...
    console.warn('syncTripsDown exception:', e.message);
    return null;
  }
}
...
export async function syncMemosUp(userId, memos) {
  ...
  try {
    if (!memos?.length) {
      ...
      if (error) console.warn('syncMemosUp clear error:', error.message);
      return;
    }
    ...
    if (error) console.warn('syncMemosUp error:', error.message);
  } catch (e) {
    console.warn('syncMemosUp exception:', e.message);
  }
}
...
export async function syncMemosDown(userId) {
  ...
  try {
    ...
    console.warn('syncMemosDown exception:', e.message);
    return null;
  }
}
 ...
 * 删除一个旅程及其关联数据。
 *
 * userId 为空即游客模式：这些旅程从来只存在本机，没有云端副本可删。
 * 早先这里硬性要求 userId，导致游客根本删不掉自己建的旅程 —— 那既是明显的
 * 功能缺陷，也正是 App Review 5.1.1 所指的「把非账号功能锁在登录之后」。
 */
export async function deleteTripAndRelated(userId, tripId) {
  if (!tripId) throw new Error('deleteTripAndRelated: missing tripId');

  const hasCloudCopy = Boolean(userId);
  ...
  return { deletedTrip: true, deletedPackingLists: hasCloudCopy, deletedLocalMemos };
}

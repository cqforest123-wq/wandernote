...
import { parseExifCoords } from './tripFromPhotos';
...
export async function attachPhotoLocations(assets, { onProgress } = {}) {
  ...
  for (let i = 0; i < list.length; i += 1) {
    const asset = list[i];
    // Ask whether coordinates can actually be read, not whether a key exists.
    // After stripping, iOS still hands over `GPS: {}` — an empty dict, which is
    // truthy — so a key-presence test skipped exactly the photos that needed
    // looking up. Reusing the real parser also keeps this in step with the one
    // place that decides what counts as a usable coordinate.
    const alreadyHas = parseExifCoords(asset?.exif) !== null;

...
/**
 * One line describing a day's recorded stops: how many, and how long in total.
 *
 * Returns null when there is nothing to say, so the caller can omit the row
 * rather than print a zero.
 */
export function summariseDayVisits(day) {
  const visits = (day?.visits || []).filter(
    v => Number.isFinite(v?.coords?.lat) && Number.isFinite(v?.coords?.lng)
  );
  ...
  const minutes = visits.reduce((total, visit) => total + (stayMinutes(visit) || 0), 0);

  return {
    places: visits.length,
    // Whole minutes: a stay is a rough figure and decimals imply otherwise.
    minutes: Math.round(minutes),
    hours: Math.floor(minutes / 60),
    remainderMinutes: Math.round(minutes % 60),
  };
}

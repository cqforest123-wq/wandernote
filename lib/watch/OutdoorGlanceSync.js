...
export default function OutdoorGlanceSync({ trips, loaded, language }) {
  ...
  useEffect(() => {
    ...
    Promise.all([getHomeCurrency(), loadRates()])
      .then(([homeCurrency, rates]) => {
        if (!cancelled) {
          setMoney({ homeCurrency, rates });
        }
      })
      .catch(() => {
        if (!cancelled) {
  ...
  const snapshotInput = useMemo(
    () =>
      makeOutdoorGlanceSnapshotInputFromAppState({
        ...
        language,
        todaySpendText,
        usesMetric: regionUsesMetric(),
      }),
    [trips, weather, language, todaySpendText]
  );

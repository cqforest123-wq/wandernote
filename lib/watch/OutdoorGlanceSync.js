...
export default function OutdoorGlanceSync({ trips, loaded, language }) {
  ...
  // Bumped whenever the app comes back to the foreground, so preferences the
  // user changed on another screen are re-read. Without it the currency and
  // unit settings were fetched once per cold start: switching to metric in
  // Profile left the watch on imperial until the app was killed and reopened.

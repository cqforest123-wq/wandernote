...
const RAW_DATE = value => value;

/**
 * @param formatDate turns a stored date into display text. The share card is
 * a picture someone sends on, so it should read in their locale; the default
 * returns the stored form, which is what tests and non-UI callers want.
 */
export function tripDateRange(trip, formatDate = RAW_DATE) {
  ...
  if (dates.length === 0) {
    const fallback = String(trip?.plannedDate || trip?.date || '').trim();
 ...
 */
export function buildTripShareStats(trip, { homeCurrency, rates, usesMetric = true, formatDate = RAW_DATE } = {}) {
  ...
  return {
    ...
    dateRange: tripDateRange(trip, formatDate),

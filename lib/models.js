 ...
 * "1 day" not "1 days".
 *
 * Languages that do not inflect return the same word for both keys, so this
 * stays correct everywhere without a plural-rules dependency.
 */
export function pluralUnit(t, count, oneKey, manyKey) {
  return t(Number(count) === 1 ? oneKey : manyKey);
}

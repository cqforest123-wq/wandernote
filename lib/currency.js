...
import { FALLBACK_HOME_CURRENCY, homeCurrencyForRegion } from './currencyMath';
...
/**
 * The raw measurement system the device reports, for diagnostics.
 *
 * This follows the device *region*, which is not the device language: an
 * iPhone set to United States reports imperial whatever language the app runs
 * in, which is how a user in Chengdu read altitude in feet.
 */
export function regionMeasurementSystem() {
...
/** Whether the device's region is metric, ignoring any explicit preference. */
export function regionUsesMetric() {
  const system = regionMeasurementSystem();

  if (system.startsWith('metric-flag:')) {
    // Older expo-localization exposes the flag instead of the system name.
    return system !== 'metric-flag:false';
  }
...
/**
 * Metric or not, after the user's explicit choice and then the device region.
 *
 * One place, because two callers deciding this separately is how they end up
 * disagreeing.
 */
export async function resolveUsesMetric() {
  const choice = await getUnitPreference();

  if (choice === 'metric') return true;
  if (choice === 'imperial') return false;

  return regionUsesMetric();
}
...
/** The device region's currency, if the app offers it; otherwise USD. */
export function regionHomeCurrency() {
  try {
    return homeCurrencyForRegion(getLocales()?.[0]?.currencyCode);
  } catch (e) {
    return FALLBACK_HOME_CURRENCY;
  }
}
...
export async function getHomeCurrency() {
  try {
    ...
    return saved || regionHomeCurrency();
  } catch (e) {
    return regionHomeCurrency();
  }
}

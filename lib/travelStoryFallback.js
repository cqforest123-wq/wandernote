...
export function buildLocalItinerary({ destination, days = 5, style = 'balanced' } = {}) {
  ...
  const plans = Array.from({ length: dayCount }, (_, index) => {
    const day = index + 1;
    return [
      `📅 Day ${day} · ${dest} ${styleText} route`,
      `🌅 Morning: Start with an easy landmark, neighborhood walk, or local cafe near your stay.`,
      `☀️ Afternoon: Visit one main attraction and leave time for transit, meals, and notes.`,
      `🌙 Evening: Keep the schedule flexible for dinner, a viewpoint, or a relaxed walk.`,
      `📍 Distance: Keep routes compact and group nearby places together.`,
      `⏱ Duration: Plan 2-3 focused blocks instead of rushing the whole day.`,
      `🏪 Opening status: Check hours locally before departure.`,
      `💡 Tips: Save addresses, tickets, weather notes, and packing reminders in WanderNote.`,
    ].join('\n');
  }).join('\n\n');

  return [
    `🗺 ${dest} ${dayCount}-Day Travel Plan`,
    '',
    '⚠️ This local plan was generated without the online AI service. Please verify details before departure.',
    '',
    plans,
  ].join('\n');
}

...
/** Used when the device region names no currency the app offers. */
export const FALLBACK_HOME_CURRENCY = 'USD';

/**
 * The home currency to assume before the user picks one.
 *
 * This was CNY for everyone, so a first-time user in Berlin or Boston saw their
 * trip totals in yuan. The device region already knows better; units have
 * followed it for a while. A region whose currency is not on the list falls
 * back to USD, the rates table's own base, rather than to a guess.
 */
export function homeCurrencyForRegion(regionCurrencyCode) {
  const code = typeof regionCurrencyCode === 'string' ? regionCurrencyCode.toUpperCase() : '';

  return COMMON_CURRENCIES.some(item => item.code === code) ? code : FALLBACK_HOME_CURRENCY;
}

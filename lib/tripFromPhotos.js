...
import { offlineReverseGeocode } from './offlineReverseGeocode';
...
// How long the whole Nominatim fallback may take. Where the host is
// unreachable every lookup costs the full timeout, and the import is a
// foreground flow the user is watching.
const NOMINATIM_BUDGET_MS = 8000;
 ...
 */
export async function buildTripDraftFromPhotos(
  assets,
  t,
  { onProgress, language = 'en' } = {}
) {
  ...
  // photos were taken; failing to find a *label* for that spot is no reason to
  // throw the position away — the map, the footprint and the destination
  // weather all need the position, not the name.
  for (const day of daysWithCoords) {
    day.center = averageCoords(day.coordsList);
  }

  const noteNamed = (day, place) => {
    if (!place) return;
    day.place = place;
    geocodedDays++;
  ...
  // Apple first, and all days at once. Apple's geocoder carries no
  // one-request-per-second condition, so serialising it bought nothing and
  // cost a multiple of the slowest lookup.
  onProgress && onProgress({ done: 0, total: daysWithCoords.length });

  const appleResults = await Promise.all(
    daysWithCoords.map((day) =>
      reverseGeocodeApple(day.center.lat, day.center.lng).catch((error) => {
  ...
  appleResults.forEach((place, i) => noteNamed(daysWithCoords[i], place));
  onProgress && onProgress({ done: geocodedDays, total: daysWithCoords.length });

  // Only what Apple could not name falls through to Nominatim, which does have
  // a one-per-second condition and so stays serial. Bounded overall: three
  // unreachable-host timeouts in a row used to hold the import for twenty
  // seconds with nothing on screen.
  const unnamed = daysWithCoords.filter((day) => !day.place);
  const nominatimDeadline = Date.now() + NOMINATIM_BUDGET_MS;

  for (let i = 0; i < unnamed.length; i++) {
    if (Date.now() > nominatimDeadline) {
      logEvent('photo-import', 'geocode-budget-spent', {
        remaining: unnamed.length - i,
      });
      break;
    }

    const day = unnamed[i];
    const place = await reverseGeocodeNominatim(
      day.center.lat,
      day.center.lng
    ).catch((error) => {
      logEvent('photo-import', 'geocode-failed', {
    ...
    noteNamed(day, place);
    onProgress &&
      onProgress({ done: geocodedDays, total: daysWithCoords.length });

    if (i < unnamed.length - 1) await sleep(GEOCODE_INTERVAL_MS);
  }

  // Whatever neither service could name gets the nearest well-known city from
  // the bundled table. Offline, instant, and it never sends a coordinate
  // anywhere — the point is that a trip should not be left nameless just
  // because the device cannot reach a geocoder.
  for (const day of daysWithCoords) {
    if (day.place) continue;
    noteNamed(day, offlineReverseGeocode(day.center.lat, day.center.lng, language));
  }

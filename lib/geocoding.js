...
export async function reverseGeocode(lat, lng, language = 'en') {
  ...
  // Last resort, and the only one that works offline. Both online geocoders
  // fail together in mainland China — Nominatim is unreachable and Apple's is
  // served by AutoNavi, which does not cover places abroad — so a photo import
  // from Hawaii came back with good coordinates and no name whatsoever.
  return offlineReverseGeocode(lat, lng, language);
}

...
export async function importBackup() {
  ...
  const merged = mergeBackup({
    ...
    currentMemos,
    incomingTrips,
    incomingMemos: data.memos,
  });

  await AsyncStorage.multiSet([
    [STORAGE_KEYS.trips, JSON.stringify(merged.trips)],
    [STORAGE_KEYS.memos, JSON.stringify(merged.memos)],
  ]);

  return {
    trips: merged.trips,
    addedTrips: merged.addedTrips,
    skippedTrips: merged.skippedTrips,
    addedMemos: merged.addedMemos,
    skippedMemos: merged.skippedMemos,
    restoredPhotos: Object.keys(photoMap).length,
  };
}

/**
 * Merge an imported backup into what is already on the device.
 *
 * Pure and exported so the one rule that matters here can actually be tested:
 * nothing already on the device is ever replaced. Travel records are the thing
 * a user can least afford to lose, so an id that already exists is skipped and
 * local data always wins. A full restore means importing into an empty app.
 */
export function mergeBackup({
  ...
  const existing = Array.isArray(currentTrips) ? currentTrips : [];
  const existingMemos = Array.isArray(currentMemos) ? currentMemos : [];
  const arriving = Array.isArray(incomingTrips) ? incomingTrips : [];
  const arrivingMemos = Array.isArray(incomingMemos) ? incomingMemos : [];

  const tripIds = new Set(existing.map((trip) => String(trip?.id)));
  const newTrips = arriving.filter((trip) => !tripIds.has(String(trip?.id)));

  const memoIds = new Set(existingMemos.map((memo) => String(memo?.id)));
  const newMemos = arrivingMemos.filter((memo) => !memoIds.has(String(memo?.id)));

  return {
    trips: [...existing, ...newTrips],
    memos: [...existingMemos, ...newMemos],
    addedTrips: newTrips.length,
    skippedTrips: arriving.length - newTrips.length,
    addedMemos: newMemos.length,
    skippedMemos: arrivingMemos.length - newMemos.length,
  };
}
